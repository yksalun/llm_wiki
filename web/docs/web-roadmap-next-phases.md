...
> 状态：已完成。已按 [spec](../../docs/superpowers/specs/2026-04-26-ui-chinese-dark-theme-design.md) 和 [implementation plan](../../docs/superpowers/plans/2026-04-26-ui-chinese-dark-theme.md) 落地，覆盖浏览器可见 UI 中文化、内部契约显示映射、公开错误消息中文化、浅色 / 深色主题切换、深色主题语义变量和关键测试。

这不是新的 roadmap 功能阶段，而是面向当前 Web 工作台体验的一次横向补强。内部 API 字段、路由、类型、枚举值、文件路径和项目数据保持原样，只在显示层输出中文。
